#!/usr/bin/env node
/** Google Indexing API — explicit URLs only (capetown-invest.com). */
import { GoogleAuth } from 'google-auth-library';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { recordSubmitted } from './lib/record-submitted.mjs';
...
if (!urls.length) {
  console.error('Usage: node scripts/submit-google-explicit.mjs https://capetown-invest.com/...');
  process.exit(1);
}
...
if (okUrls.length) {
  const r = recordSubmitted({ siteFolder: 'capetown-invest-website', urls: okUrls, channel: 'google' });
