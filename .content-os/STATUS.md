...
**Следующие волны:** R4 — `segments/` (4 страницы, среднее 13.2, три нуля) и `developers/` (7 страниц, среднее 26.1, три нуля); это худшее, что осталось. R5 — Century City и 28 страниц проектов. R6 — оставшиеся гайды плюс три пары каннибалов (`rental-yield-guide` + `highest-rental-yield-suburbs`, `new-developments` + `off-plan`, `southern-suburbs` + `stellenbosch`).
