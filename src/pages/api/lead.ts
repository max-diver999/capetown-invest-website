...
const LEAD_FROM_EMAIL = import.meta.env.LEAD_FROM_EMAIL || `Cape Town Invest <${SITE.email}>`;
...
async function sendAutoReply(to: string, name: string, context: string) {
  ...
  const topic = context ? escapeHtml(context) : 'your Cape Town property enquiry';
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const payload = isHealthcheck
      ? { success: true, accepted: true, notify: { telegram: telegramFailure || 'ok', email: notifyFailure || 'ok' } }
