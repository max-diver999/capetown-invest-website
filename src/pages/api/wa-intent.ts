...
const LEAD_FROM_EMAIL =
  import.meta.env.LEAD_FROM_EMAIL || `Cape Town Invest <${SITE.email}>`;
...
function normalizeSiteUrl(value: unknown): string {
  ...
  try {
    ...
    if (host !== 'capetown-invest.com' && !host.endsWith('.vercel.app')) {
      return '';
    }
    return url.href;
  } catch {
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const lines = [
      '💬 <b>WhatsApp click | capetown-invest.com</b>',
    ...
    const plainLines = [
      'WhatsApp click | capetown-invest.com',
      `Ref: ${refCode}`,
    ...
    try {
      await sendLeadNotifyEmail({
        subject: `WhatsApp click — capetown-invest.com (${refCode})`,
        htmlBody: plainLines,
