...
That gap creates two practical problems. Bond approvals have a shelf life, and on a build running toward the far end of the 12 to 36 month range, your approval will lapse. The bank then reassesses your income, credit record, and the unit's valuation at that later date rather than at signing. The second problem is pricing: the rate you actually pay is the rate in force at registration, not the one indicated at launch.
...
The downside is less obvious. Scarcity removes the discipline a crowded launch market imposes on developers: fewer competing schemes means weaker incentives at launch and more confidence to push asking prices. It also tempts developers without secured finance to launch on the strength of a sales target, which is the precise failure mode that the trust account, NHBRC enrolment, and long-stop date checks exist to catch. A tightening pipeline is a reason to move deliberately, never a reason to shorten diligence.
