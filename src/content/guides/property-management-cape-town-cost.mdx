...
The table below extends the Sea Point worked example from the [gross vs net yield guide](/guides/gross-vs-net-yield-cape-town/) by adding each management layer in turn: the 10% collection fee, amortised tenant placement, and a maintenance markup reserve. All figures are modeled rather than guaranteed, and the point is the sequence, not any single line.
...
Read the fee base rather than the fee number. A long-term manager taking 8% to 10% of collected rent charges on money that actually arrived, against a vacancy assumption of 8% to 10% a year. A short-term manager taking 15% to 20% of gross bookings charges on revenue before a 25% to 40% vacancy allowance and before cleaning at R350 to R800 per turnover, so the effective drag on collected income runs well above the headline. Modeled on the same Sea Point unit, long-term management lands net near 6.5% to 6.6% while short-term management lands in a volatile mid-single-digit band. Add the regulation difference and the comparison sharpens further: a body corporate can end the short-term model with a 75% special resolution, and no fee structure protects against that.
