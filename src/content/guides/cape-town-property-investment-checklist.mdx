...
Only one purchase tax applies to a Cape Town property, never both. Resale stock carries transfer duty, a national SARS tax paid by the buyer: 0% up to R1,210,000, then 3%, 6%, 8%, 11%, and 13% on the portions above each threshold, so a R3,000,000 home pays roughly R107,355. A new build sold by a VAT-registered developer carries 15% VAT instead, and that VAT already sits inside the advertised price, so no transfer duty is due on it. Confirm in the Offer to Purchase which of the two applies before you sign, because getting it wrong changes your all-in cost materially. A new build quoted as duty-applicable, or a resale quoted as VAT-inclusive, is a paperwork error that costs tens of thousands and is far easier to fix before signature than after.
...
Exit tax on a Cape Town sale has two moving parts, and non-residents meet both of them. Capital gains tax applies to the profit: an individual includes 40% of the gain in taxable income while a trust or company includes 80%, and a primary residence carries an exclusion of up to R2,000,000 on the gain. Separately, section 35A obliges the conveyancer to withhold a slice of the price when a non-resident seller sells above R2,000,000: 7.5% for an individual, 10% for a company, 15% for a trust. That withholding is a cash-flow event on transfer day, credited against the final CGT bill rather than an extra tax on top. Keep every cost record from this checklist, because transfer duty, conveyancing, levies, and improvements raise base cost and shrink the taxable gain.
