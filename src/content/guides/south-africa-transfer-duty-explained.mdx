...
readingTime: 14
heroImage: "https://res.cloudinary.com/dlrrtf6bq/image/upload/v1787356375/more-group/capetown/guides/south-africa-transfer-duty-explained.jpg"
heroAlt: "The sun setting over Hout Bay from Chapman's Peak Drive"
faq:
...
South Africa transfer duty is a national tax the buyer pays to SARS whenever immovable property changes ownership, and it is the largest once-off cost in most purchases. Nothing is due below R1,210,000; above that the rate steps from 3 percent up to 13 percent on the slice above R13,310,000, calculated on the price in the Offer to Purchase rather than the municipal valuation.
...
Foreign buyers pay exactly the same South Africa transfer duty as citizens, with no surcharge, no premium band and no separate non-resident scale. A German or British buyer signing at R5,000,000 writes the same R327,356 duty cheque as a Cape Town local, and the R1,210,000 zero-rated threshold applies identically. Where non-residents genuinely differ is finance and exit. South African banks typically lend non-residents up to about 50 percent of value, so the cash portion of the deal is larger and the duty must be funded from that cash side before registration. On the eventual resale, section 35A obliges the conveyancer to withhold 7.5 percent of the price for an individual non-resident seller, 10 percent for a company and 15 percent for a trust, on any sale above R2,000,000. Budget duty as a local would, then plan the currency and withholding layers separately.
...
Transfer duty is paid during conveyancing, after the Offer to Purchase is signed and before the Deeds Office registers the property in your name. SARS requires payment within 6 months of the signature date, and a Cape Town registration usually completes in two to three months, so on a R5,000,000 purchase the R327,356 is requested well inside the window.

- Signature of the Offer to Purchase starts the 6 month SARS clock
- The conveyancer calculates the duty and calls for it with the purchase funds
- SARS issues the transfer duty receipt the Deeds Office requires before registration

Once conditions are met, the transferring attorney prepares the transfer documents, calculates the duty, and requests the amount from you along with the purchase funds held in trust.
...
Insider tip: the six-month SARS clock runs from the date the last party signs the Offer to Purchase, not from the date your bond is approved or your suspensive conditions finally lapse. On sales where the seller is waiting on a subdivision, a body corporate consent or a deceased estate to be wound up, that clock can quietly expire while nothing looks wrong, and interest then accrues on the duty. Ask the conveyancer in writing which signature date they are counting from, and diarise month five.
...
Transfer duty dominates the once-off buying budget from about R3,000,000 upwards, and planning it first is the difference between a realistic offer and a stalled transfer. On a R2,000,000 resale the R33,786 duty is roughly 54 percent of the total add-on cost, so the conveyancing and Deeds Office fees around it still feel material. At R5,000,000 the R327,356 duty is about 86 percent of the add-on stack, and at R10,000,000 the R877,356 duty is about 92 percent, which turns every other fee into rounding. A cash buyer on a VAT-inclusive new build avoids the duty line entirely, while a bonded buyer adds bond registration on top. Work out the duty for your exact price using the SARS brackets, add roughly R28,000 to R76,000 for the surrounding fees, and only then decide what you can bid.
...
South Africa transfer duty is progressive and marginal: nothing below R1,210,000, then 3 percent to 13 percent applied slice by slice rather than to the whole price. A R2,000,000 resale costs R33,786, a R5,000,000 home costs R327,356, and a R10,000,000 home costs R877,356, which is why the duty line gets planned before the offer rather than after acceptance.
...
Three transfer duty errors should pause a Cape Town purchase. A rounded 10 percent all-in rule of thumb instead of the SARS brackets misprices every deal above R2,994,800, where 11 percent applies to the slice. Forgetting that duty is due on a cash purchase leaves a R327,356 hole on a R5,000,000 buy. Off-plan payment structures that blur which amount triggers duty timing are the third.
...
## Which buyer profile fits transfer duty math?

Buyer profile changes the transfer duty math more than the property type does. A first-time buyer under R1,210,000 pays nothing at all. A family buyer at R5,000,000 writes a R327,356 cheque before registration. A new-build buyer from a VAT-registered developer pays 15 percent VAT inside the price and no duty. A foreign buyer pays exactly what a local pays.

| Buyer profile | Price point | Transfer duty |
| --- | --- | --- |
| First-time entry buyer | R1,200,000 | R0, below the threshold |
| Family resale buyer | R5,000,000 | R327,356 |
| Prime Atlantic Seaboard buyer | R10,000,000 | R877,356 |
| New build from VAT-registered developer | Any price | R0, 15 percent VAT in the price |
| Non-resident foreign buyer | R5,000,000 | R327,356, identical to a local |

The practical consequence is that duty belongs in the offer calculation, not in the closing paperwork. A price negotiated from R3,050,000 down to R2,990,000 saves R60,000 on price and drops the balance below the 11 percent line, and a buyer comparing a VAT-inclusive new build against a resale at the same headline price is comparing two different totals.
