...
Underwrite enrolment before construction, not after handover, because an unenrolled home has no warranty fund backstop if the builder fails. Deadline risk means all three periods run from occupation, so structural defects near year 5 must be reported in writing before the clock expires. Builder solvency risk shifts repair from the developer to an NHBRC claim, which requires enrolment certificate, occupation date, and dated defect correspondence. Documentation risk stalls valid claims when buyers lose the paper trail.
...
Stop when a developer pitch lacks NHBRC enrolment proof for the specific phase you are buying, when snag lists are deferred to after registration without a written defect window, or when marketing visuals do not match enrolled plans filed with the NHBRC. Off-plan buyers pay 10 to 30 percent in deposits within 3 to 6 months of signing while the unit does not yet exist, so enrolment in writing before the first transfer is the baseline protection against developer or builder failure.
