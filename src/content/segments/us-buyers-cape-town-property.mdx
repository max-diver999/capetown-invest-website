...
Three pulls dominate American files, and the first one is rarely the one buyers lead with in conversation.

The dollar is the honest driver. A soft rand means a budget that buys a modest condo in a US coastal city reaches prime Atlantic-facing stock here, and nothing pads that entry price the way a foreign-buyer surcharge would in Singapore or several Australian states. The carrying cost surprises Americans too: municipal rates in Cape Town exempt the first R450,000 of valuation and run at roughly 0.69 cents per rand per year above it, which reads mildly against property tax bills in most US states.
...
A typical American pack runs to four items:
