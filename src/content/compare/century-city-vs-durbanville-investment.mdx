...
readingTime: 14
heroImage: "https://res.cloudinary.com/dlrrtf6bq/image/upload/v1787356239/more-group/capetown/compare/century-city-vs-durbanville-investment.jpg"
heroAlt: "The Hottentots Holland mountains above the Helderberg basin"
relatedSlugs:
...
Reading the two yield lines correctly means starting from the net, not the gross. Century City sectional title models roughly 7.7% gross and mid 5% to low 6% net, so the levy stack absorbs close to 2 percentage points, and that levy is a single knowable figure the body corporate publishes each year. Durbanville family homes model about 6% gross and 4.5% net, and the 1.5 point spread hides a less predictable set of costs: garden service, pool service, armed response, and roof or damp work on 1980s stock. The apartment gives you a higher number and a tighter cost forecast; the house gives you a lower number and 18 to 24 month tenant retention. Model both with the same vacancy and management assumptions before you compare them.
...
Transfer duty runs on the same SARS scale in both nodes, so the difference lands on the size of the ticket rather than on the buyer's passport. Nothing is payable on the first R1,210,000, and the rate then steps through 3%, 6%, 8%, 11%, and 13% on the slices above it, which means a R2 million Century City one-bedroom clears the first band cheaply while a R3.5 million to R5.5 million Durbanville estate house climbs into the higher rungs on identical rules. Non-residents borrow up to about 50% locally in both nodes, so the balance arrives from offshore and must be recorded through an authorised dealer bank at entry. On exit above R2,000,000, section 35A withholds 7.5% for an individual, 10% for a company, and 15% for a trust.

Insider tip: in Century City ask for both levies, not one. Sectional title owners pay a body corporate levy for their own building and a separate precinct contribution toward the shared security, roads, and landscaping that make the node work, and buyers routinely model only the first. On Durbanville freehold the trap runs the other way: no levy appears on the statement at all, but armed response, garden service, and pool service are real monthly costs that never show up on a listing.
...
## What risks should you plan for with Century City vs Durbanville?
...
## Corporate Income vs Family Long-Let Stability
