...
Kalk Bay housing stock divides into three distinct tiers based on location, views, and building condition, each offering different risk-return profiles for investors. The split matters more here than in a uniform apartment suburb: built-area pricing runs from about R22,000 per square metre for an unrenovated village cottage to R65,000 for a waterfront position, and gross yield moves the other way, from 7.5% at the entry tier down to 5% at the top.
...
Kalk Bay's investment appeal stems from village infrastructure supporting walkable lifestyle rare in car-dependent Cape Town suburbs. Harbour, restaurants, galleries, and the train station sit inside a 500-metre radius, so a car-free tenant can live here, which widens the letting pool beyond what a Southern Peninsula address usually reaches. The gap is retail: no full-line grocer trades in the village, and that omission is what keeps Kalk Bay a lifestyle address rather than a family base.
