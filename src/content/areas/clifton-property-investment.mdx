...
Insider tip: in Clifton the beach line and the road line are two different markets, and averaging them will mislead your offer. Cape Town Invest treats a Clifton valuation as a beach-access question first and a square-metre question second, because the R80,000 to R180,000 per square metre band spans both sheltered beach frontage and inland stock on the Victoria Road side. Ask for transacted comparables from the same beach and the same access path, not merely the same suburb. Yields here are MODELED near 5% gross and 3.5% net, the thinnest on the Atlantic Seaboard, so income will never rescue an overpriced entry. The liquidity argument is the real one: luxury sales above R20m across the strip rose 61% to R4.2bn in 2025 and foreign buyers took about 25% of value, which is what makes a trophy Clifton exit workable when you need it.
