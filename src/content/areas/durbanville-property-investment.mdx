...
Transfer duty runs on the SARS scale that applies from 1 April 2025, and it is one reason the Durbanville entry ticket stays affordable in absolute terms. Nothing is payable on the first R1,210,000, and the rate then steps through 3%, 6%, 8%, 11%, and 13% on the slices above it, which is how a R4M family home lands near R167,500, roughly 4.2% of price, while coastal stock at two or three times the value reaches the top bands. Non-resident sellers carry a separate cash cost on exit: on any sale above R2,000,000 the conveyancer must withhold 7.5% of the price for an individual, 10% for a company, and 15% for a trust under section 35A, credited later against the capital gains assessment rather than lost.
